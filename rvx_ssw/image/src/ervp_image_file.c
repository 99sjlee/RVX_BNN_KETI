...
static int _rgb888_write_file(const ErvpImage* rgb888, const char *filename, int options)
{
  int w = rgb888->width;
  int h = rgb888->height;
  int c = 3;
...
int image_file_write(const ErvpImage* image, const char *filename, int options)
{
  assert(image!=NULL);
  int success = 0;
  if(image->format==IMAGE_FMT_RGB_888_PACKED)
    success = _rgb888_write_file(image, filename, options);
